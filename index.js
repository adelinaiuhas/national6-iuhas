console.log('to-do-app-homework-aiuhas')
...
document.getElementById("add-task-button").addEventListener("click", postData);
...
function renderToDoList(payload) {
  ...
  taskContent.innerText = payload.item;

  removeArticle.addEventListener("click", function () {
    console.log("remove element");
    // this will remove the entire post from the DOM
    task.remove();

    // this wil remove the post from the server as well
    fetch(
      "https://simple-json-server-scit.herokuapp.com/todo/aiuhas",
      {
        method: "DELETE",
      }
      // getData it is reuse so that you get the new data from server
    ).then(getData);
  });
